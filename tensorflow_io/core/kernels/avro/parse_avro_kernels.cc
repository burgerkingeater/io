...
Status ParseAvro(const AvroParserConfig& config,
  ...
  const size_t num_minibatches = [&] {
    ...
    if (const char* n_minibatches = std::getenv("AVRO_PARSER_NUM_MINIBATCHES")) {
      VLOG(5) << "Overriding num_minibatches with " << n_minibatches;
      result = std::stoi(n_minibatches);
    }
    // This is to ensure users can control the num minibatches all the way down to size of 1(no parallelism).
